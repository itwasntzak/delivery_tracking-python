...
def startShift():
    with open(os.path.join("deliveryTracking", "shifts", str(utilFunc.now().date()) + '.py'), 'w') as today:
        today.write('startShift = ' + str(utilFunc.now()))


def endShift():
    with open(os.path.join("deliveryTracking", "shifts", str(utilFunc.now().date()) + '.py'), 'a+') as today:
        content = today.read()
